...
class Volumes(generic.View):
    ...
    def get(self, request):
        ...
        if request.GET.get('all_projects') == 'true':
            ...
            result, has_more, has_prev = api.cinder.volume_list_paged(
                request, search_opts=search_opts, **kwargs)
        return {
    ...
    def post(self, request):
        ...
        return rest_utils.CreatedResponse('/api/cinder/volumes/%s' % volume.id,
                                          volume.to_dict())
...
class VolumeTypeMetadata(generic.View):
    ...
    def patch(self, request, volume_type_id):
        ...
        if removed:
            api.cinder.volume_type_extra_delete(request, volume_type_id,
                                                removed)
...
# custom for bls
@urls.register
class CreateVolumeSnapshot(generic.View):
    url_regex = r'cinder/volumes/(?P<volume_id>[^/]+)/create_snapshot/$'
    ...
    def post(self, request, volume_id):
        ...
        data['force'] = True
        result = api.cinder.volume_snapshot_create(request, **data)
...
class VolumeSnapshot(generic.View):
    # api/cinder/volumesnapshots/" + id + "/"
    url_regex = r'cinder/volumesnapshots/(?P<volume_snapshot_id>[^/]+)/$'
    ...
    @rest_utils.ajax()
    def post(self, request, volume_snapshot_id):
        name = request.DATA['name']
        description = request.DATA['description']
        return api.cinder.volume_snapshot_update(request, volume_snapshot_id,
                                                 name, description)
