    ...
    def post(self, request, server_id):
        ...
        for con_type, api_call in check_consoles.items():
            ...
            if con_type == 'SERIAL':
                ...
                console_url = "%s&%s(%s)" % (
                    console.url, utils_http.urlencode({'title': _("Console")
                                                       }), server_id)
...
class ConsoleOutput(generic.View):
    ...
    def post(self, request, server_id):
        ...
        console_lines = api.nova.server_console_output(request,
                                                       server_id,
...
class ServerGroups(generic.View):
    ...
    def post(self, request):
        ...
        return rest_utils.CreatedResponse(
            '/api/nova/servergroups/%s' %
            utils_http.urlquote(new_servergroup.id), new_servergroup.to_dict())
...
class Flavors(generic.View):
    ...
    def post(self, request):
        ...
        flavor = api.nova.flavor_create(
            request,
            name=request.DATA['name'],
            memory=request.DATA['ram'],
            vcpu=request.DATA['vcpus'],
            disk=request.DATA['disk'],
            ephemeral=request.DATA['OS-FLV-EXT-DATA:ephemeral'],
            swap=request.DATA['swap'],
            flavorid=flavor_id,
            is_public=is_public)

        for project in flavor_access:
            api.nova.add_tenant_to_flavor(request, flavor.id,
                                          project.get('id'))
...
class Flavor(generic.View):
    ...
    def get(self, request, flavor_id):
        ...
        if get_access_list and not flavor.is_public:
            access_list = [
                item.tenant_id
                for item in api.nova.flavor_access_list(request, flavor_id)
            ]
...
class FlavorExtraSpecs(generic.View):
    ...
    def patch(self, request, flavor_id):
        ...
        if request.DATA.get('removed'):
            api.nova.flavor_extra_delete(request, flavor_id,
                                         request.DATA.get('removed'))
...
class DefaultQuotaSets(generic.View):
    ...
    def patch(self, request):
        ...
        if api.base.is_service_enabled(request, 'compute'):
            ...
            filtered_quotas = [
                quota for quota in quotas.NOVA_QUOTA_FIELDS
                if quota not in disabled_quotas
            ]

            request_data = {
                key: request.DATA.get(key, None)
                for key in filtered_quotas
            }

            nova_data = {
                key: value
                for key, value in request_data.items() if value is not None
            }
...
class EditableQuotaSets(generic.View):
    ...
    def get(self, request):
        ...
        disabled_quotas = quotas.get_disabled_quotas(request)
        editable_quotas = [
            quota for quota in quotas.QUOTA_FIELDS
            if quota not in disabled_quotas
        ]
...
# custom for bls
@urls.register
class AttachVolume(generic.View):
    url_regex = r'nova/servers/(?P<server_id>[^/]+)/volumes/(?P<volume_id>[^/]+)/attach/$'
...
@urls.register
class DetachVolume(generic.View):
    url_regex = r'nova/servers/(?P<server_id>[^/]+)/volumes/(?P<volume_id>[^/]+)/detach/$'
